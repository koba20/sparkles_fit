...
const Men = () => {
  ...
  // Filter products to only show men's products
  const menCategories = categories.filter(
    (cat) =>
      cat.name.toLowerCase().includes("men") ||
      cat.name.toLowerCase().includes("mens")
  );

  const menCategoryIds = menCategories.map((cat) => cat.id);

  const menProducts = products.filter((product) =>
    menCategoryIds.includes(product.category_id)
  );

  const filteredProducts =
    selectedCategory === "all"
      ? menProducts
      : menProducts.filter((p) => p.category_id === selectedCategory);
  ...
  return (
    <>
      <Navbar />
      <div className="min-h-screen pt-16 bg-gradient-to-br from-blue-50 to-blue-100">
        {/* Hero Section */}
        <div className="relative h-[60vh] bg-gradient-to-r from-blue-600 to-blue-800 flex items-center justify-center">
          <div className="absolute inset-0 bg-blue-900/20"></div>
          <div className="relative text-center text-white">
            <h1 className="text-6xl font-light tracking-[0.3em] mb-4">MEN'S</h1>
            ...
            {filteredProducts.map((product) => (
              ...
              >
                  ...
                  <div className="relative aspect-[3/4] overflow-hidden bg-card mb-4 rounded-t-lg">
                    ...
                    >
                      <Button
                        size="icon"
                        variant="secondary"
                        className="bg-white/90 text-blue-600 hover:bg-white w-10 h-10"
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                        }}
                ...
                <div className="p-4 space-y-2">
                  <h3 className="text-lg font-medium tracking-wide text-gray-900 group-hover:text-blue-600 transition-colors duration-300">
                    {product.name}
                  </h3>
